...
OPS = (OP_AUTH_URL, OP_SIGN_IN, OP_VOICE_START, OP_VOICE_STATUS, OP_VOICE_CLEAR,
       ...
       OP_ACCOUNT_FORGET, OP_CHECKOUT_URL, OP_CHECKOUT_CONFIRM, OP_PORTAL_URL)
...
def checkout_url(account_id, fallback="/dashboard"):
    """Where to send this account to pay, and the id of the checkout minted for
    them: `(url, checkout_id)`, the url being `fallback` when none could be
    minted. Minting one is a POST carrying the organization token, so it happens
    in the mail role.

    The id crosses back because the caller records it in the buyer's own
    browser, and the return trip is refused unless that browser is the one that
    started this checkout. None on the static fallback, which mints nothing to
    name."""
    result = call(OP_CHECKOUT_URL, account_id=account_id, fallback=fallback)
    return result["url"], result["checkout_id"]
