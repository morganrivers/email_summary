...
def all_accounts():
    ...
    Every check here reads the manifest file rather than an argument, which is
    why every one of them raises. The duplicate-handle one is the control
    stopping one account from opening another's records, and a control spelled
    as an assert is a control with `-O` for an off switch."""
    data = _read_manifest()
    if data is None:
        raise InvalidAccountData(
    ...
    if len(customers) != len(set(customers)):
        raise InvalidAccountData(
            ...
            "returns whichever the manifest lists first and the reconcile "
