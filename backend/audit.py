...
def prune(before=None):
    ...
    Safe to run at any time, and that is the whole difference from the
    co-signer's. Nothing reads this table to decide anything -- it is written by
    the mutators and read by a human -- so retention here is a policy dial.
    `cosigner/retention.py` is a module of its own, with cutoffs derived from
    the limiter's longest window and an assertion refusing to run if that stops
    holding, because its log doubles as the rate limiter's counter and its
    wrap-once state had to be split into a second table before a row could be
    deleted at all."""
