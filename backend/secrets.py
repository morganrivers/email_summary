...
# Names the Node bridge reads for the same values (gmail_lib.mjs
# loadOAuthKeys). One OAuth app serves every user, and its client_secret is the
# single value with the widest blast radius, so it is injected rather than read
# off the volume like the rest.
...
def google_oauth_client():
    ...
    injected environment, or ``None`` when it was not injected. Outside a TEE
    the Node bridge still falls back to ``gcp-oauth.keys.json``; under
    TEE_REQUIRED there is no such file and this is the only source."""
...
def google_oauth_configured():
    """Reason a user cannot complete Google consent, or None."""
