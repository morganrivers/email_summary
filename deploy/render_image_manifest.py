...
# role -> the entry-point module it starts, which is that service's `command:`
# in docker-compose.yml. One each: the images set no Entrypoint and no Cmd, so a
# container runs exactly one module and this table is the partition of them. The
# union is asserted equal to reachability.enclave_roots(), read out of that same
# compose file, so a service pointed at a module with no home here fails the
# render rather than shipping nowhere.
#
# Two kinds of module have left this list, both for the same reason: started and
# reached are different things, and only the first belongs here. The three
# scheduled jobs are called by backend/daemons/scheduler.py, and the boot gate is
# called by the two data-holding roles from their own `__main__` now that no
# entrypoint shell runs it as a separate process. Both still ship, because the
# import walk that decides every other module finds them.
#
# `egress` runs no gate at all: it unseals nothing, so there is nothing for a
# gate to release, and it is deliberately handed no guest-agent socket to run one
# against. Its image is the proxy and the allowlist, which is the whole
...
# each role's `command:` in docker-compose.yml -- do not edit. A module belongs
# in a role because something that role starts imports it; add the import, then
# re-render.
#
...
def _assert_roots_cover_the_compose_file():
    """Every module a service's `command:` starts has exactly one role here."""
    union = set().union(*ROLE_ROOTS.values())
    started = reachability.enclave_roots()
    assert union == started, (
        "ROLE_ROOTS and docker-compose.yml disagree about the enclave's entry "
        f"points; only in ROLE_ROOTS: {sorted(union - started)}; "
        f"only in the compose file: {sorted(started - union)}"
    )
...
def render():
    _assert_roots_cover_the_compose_file()
