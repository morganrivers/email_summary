...
silently: that under TEE_REQUIRED nothing is ever read off the volume (no .env
load, no OAuth keys file, and a .env that exists at all fails the boot gate),
and that the gate's notion of "provisioned" covers every secret a service
actually needs -- SESSION_SECRET and the Polar credentials among them, which
the old four-name list omitted.
...
def test_google_oauth_client_secret_is_required(env_file, monkeypatch):
    _provision(monkeypatch)
    monkeypatch.delenv(secrets.GOOGLE_CLIENT_SECRET_ENV, raising=False)

    assert secrets.google_oauth_client() is None
    assert any(secrets.GOOGLE_CLIENT_SECRET_ENV in r for r in secrets.missing())
