...
def test_a_finished_voice_job_is_waited_out_and_dropped(store, daemon):
    voice_generation._JOBS["dana@x.com"] = {"state": "done", "started": 0, "error": None}
    assert daemon.forget("dana@x.com")[handoff.F_RESULT]["voice"] == "done"
    assert "dana@x.com" not in voice_generation._JOBS
...
def test_the_wait_fits_inside_the_window_the_web_tier_allows():
    ...
    assert voice_generation.AWAIT_TIMEOUT < handoff.TIMEOUT
...
def test_a_running_job_that_outlasts_the_wait_is_reported(store, daemon, monkeypatch):
    ...
    wait."""
    monkeypatch.setattr(voice_generation, "AWAIT_TIMEOUT", 0)
    monkeypatch.setattr(voice_generation, "AWAIT_POLL", 0)
    voice_generation._JOBS["dana@x.com"] = {"state": "running", "started": 0, "error": None}
...
def test_finished_jobs_are_swept_rather_than_held_for_the_process_lifetime():
    ...
    now = voice_generation.time.time()
    voice_generation._JOBS["stale@x.com"] = {
        "state": "done", "started": now - voice_generation.JOB_RETENTION - 1,
        "error": None}
    voice_generation._JOBS["fresh@x.com"] = {
        "state": "done", "started": now, "error": None}
    voice_generation._JOBS["busy@x.com"] = {"state": "running", "started": 0, "error": None}

    assert voice_generation.status("fresh@x.com") is not None
    assert "stale@x.com" not in voice_generation._JOBS
    assert "busy@x.com" in voice_generation._JOBS, "a running job is never swept"
