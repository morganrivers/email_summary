{
  ...
  outputs =
    ...
    let
      ...
      # proxy into an image that starts neither). Three lists now, because the
      # three containers run three entry points and a module belongs in a
      # container only if that container starts something that imports it: the
      # receiver Google posts to no longer carries the inference client or the
      # custody stack the mail role's imports drag in. See that module's
      # docstring.
      ...
      # makes three images cost the space of one plus each role's own leaves.
      mkAppCode =
        files:
        builtins.path {
          name = "email-bot-app";
      ...
      # One entrypoint, three roles, and now three images -- one per role,
      # carrying only that role's reachable code. Which role a container plays is
      # still a `command:` in docker-compose.yml, and that file (which now names
      # a different image per service) is hashed into the dstack `compose-hash`
      ...
      # across the three images; a branch for a role whose modules are absent
      ...
      # it listens on: the web UI on 8790, the Pub/Sub receiver on 8787, and the
      # mail role nothing (it is reached over the custody socket and the FIFO).
      rolePorts = {
        mail = [ ];
        web = [ "8790" ];
        hook = [ "8787" ];
    ...
    in
    {
      packages.${system} = {
        ...
        # `nix build .#images` builds all three; the default is the mail image,
